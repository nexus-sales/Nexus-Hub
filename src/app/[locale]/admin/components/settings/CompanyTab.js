...
export default function CompanyTab({ companySections, updateCompanyValue, saveCompanySection, primaryColor }) {
    const t = useTranslations('Admin.settings.company');

    return (
        <div className="space-y-8 animate-in zoom-in-95 duration-300">
            {companySections.map((section, i) => (
                <div key={section.id} className="p-6 bg-slate-50 dark:bg-slate-800/50 rounded-3xl border border-slate-200 dark:border-slate-700 space-y-4">
                    <div className="flex items-center justify-between">
                        <h4 className="text-sm font-black uppercase tracking-widest text-slate-400">{t('sectionLabel')}: {section.slug}</h4>
                        <button
                            onClick={() => saveCompanySection(section)}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="space-y-2">
                            ...
                            <input
                                ...
                                value={section.title}
                                onChange={(e) => updateCompanyValue(i, 'title', e.target.value)}
                            />
                        </div>
                        <div className="space-y-2">
                            ...
                            <input
                                ...
                                onChange={(e) => updateCompanyValue(i, 'description', e.target.value)}
                            />
                        </div>
                    </div>
                    <div className="space-y-2">
                        ...
                        <textarea
                            ...
                            onChange={(e) => updateCompanyValue(i, 'content', e.target.value)}
                        />
                    </div>
                </div>
            ))}
